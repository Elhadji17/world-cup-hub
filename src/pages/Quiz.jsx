...
export default function Quiz() {
  ...
  // Vies locales fixes pour cette partie — indépendant des vies globales
  const LIVES_PER_GAME = 3;

  const [shuffledQ]      = useState(() => shuffle(allQ).slice(0, category.count));
  const [current,          setCurrent]          = useState(0);
  const [score,            setScore]            = useState(0);
  const [lives,            setLives]            = useState(LIVES_PER_GAME);
        ...
        <div className="max-w-xl mx-auto px-4 pt-10">
          ...
          {levelUp && (
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
              ...
              className="mt-4 bg-yellow-500/20 border border-yellow-400/40 rounded-2xl px-6 py-4 text-center">
              <div className="text-3xl mb-1">🎉</div>
              <div className="text-yellow-400 font-black text-lg">Niveau {LEVELS[newLevel]?.level} débloqué !</div>
              <div className="text-sm text-gray-300">{LEVELS[newLevel]?.emoji} {LEVELS[newLevel]?.label}</div>
            </motion.div>
          )}
          </motion.div>
